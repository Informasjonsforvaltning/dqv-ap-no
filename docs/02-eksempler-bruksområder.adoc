== Eksempler på bruksområder [[Eksempler_på_bruksområder]]

Noen eksempler på når spesifikasjonen skal brukes (ikke-uttømmende):

* Beskrivelse av kvantifiserbar kvalitet: Se eksemplet under egenskapen <<Datasett_harKvalitetsmåling>>.
...
* Beskrivelse av kvalitet i form av fritekst, inkl. brukertilbakemelding på kvalitet i fritekst: Se eksempel under egenskapen <<Datasett_harKvalitetsmerknad>>.
...
* Beskrivelse av at et datasett er autoritativt: Se eksempel under egenskapen <<Datasett_harKvalitetsmerknad>>.
